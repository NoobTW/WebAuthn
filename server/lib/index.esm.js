import require$$0$1 from 'base64url';
import require$$0 from 'crypto';
import require$$3 from 'cbor';
import require$$2 from 'jsrsasign';
import require$$1 from 'elliptic';
import require$$2$1 from 'node-rsa';
import require$$0$2 from '@lapo/asn1js';

const crypto$1 = require$$0;
const cbor$4 = require$$3;
const jsrsasign = require$$2;

function hash$6(alg, data) {
	return crypto$1.createHash(alg).update(data).digest();
}

function parseAuthData$5(buffer) {
...
async function verifySignature$5(signature, data, publicKey) {
	return await crypto$1.createVerify('SHA256').update(data).verify(publicKey, signature);
}
...
const base64url$5 = require$$0$1;
const { hash: hash$5, parseAuthData: parseAuthData$4, verifySignature: verifySignature$4, COSEECDHAtoPKCS: COSEECDHAtoPKCS$4 } = common;
...
async function verifyU2FAttestation$1(ctapCredentialResponse, clientDataJSON) {
	...
	const clientDataHash = hash$5('SHA256',base64url$5.decode(clientDataJSON));
...
var cose = { 
	...
	COSE_CRV: COSE_CRV$2,
	COSE_ALG_HASH: COSE_ALG_HASH$2
};

const base64url$4 = require$$0$1;
const elliptic$1 = require$$1;
const nodeRSA$1 = require$$2$1;
const cbor$3 = require$$3;
const { hash: hash$4, parseAuthData: parseAuthData$3, verifySignature: verifySignature$3, COSEECDHAtoPKCS: COSEECDHAtoPKCS$3, base64ToPem: base64ToPem$1, getCertificationInfo: getCertificationInfo$1 } = common;
const { COSE_ALG_HASH: COSE_ALG_HASH$1, COSE_KEYS: COSE_KEYS$2, COSE_KTY: COSE_KTY$1, COSE_CRV: COSE_CRV$1, COSE_RSA_SCHEME: COSE_RSA_SCHEME$1 } = cose;
...
async function verifyPackedAttestation$1(ctapCredentialResponse, clientDataJSON){
	...
	const clientDataHash = hash$4('SHA256',base64url$4.decode(clientDataJSON));
	const signatureBase = Buffer.concat([
	...
	if(ctapCredentialResponse.attStmt.x5c){
		...
		const hashAlg = COSE_ALG_HASH$1[publicKeyCose.get(COSE_KEYS$2.alg)];
		if(publicKeyCose.get(COSE_KEYS$2.kty) === COSE_KTY$1.EC2){
			const ansiKey = COSEECDHAtoPKCS$3(publicKeyCose);

			const signatureBaseHash = hash$4(hashAlg, signatureBase);

			const ec = new elliptic$1.ec(COSE_CRV$1[publicKeyCose.get(COSE_KEYS$2.crv)]);
		...
		else if(publicKeyCose.get(COSE_KEYS$2.kty) === COSE_KTY$1.RSA){
		...
		else if(publicKeyCose.get(COSE_KEYS$2.kty) === COSE_KTY$1.OKP){
			//Probly won't work
			const x = publicKeyCose.get(COSE_KEYS$2.x);
			const signatureBaseHash = hash$4(hashAlg, signatureBase);

			const key = new elliptic$1.eddsa('ed25519');
			key.keyFromPublic(x);
...
const asn1 = require$$0$2;
const base64url$3 = require$$0$1;
const cbor$2 = require$$3;
const { hash: hash$3, parseAuthData: parseAuthData$2, verifySignature: verifySignature$2, COSEECDHAtoPKCS: COSEECDHAtoPKCS$2, base64ToPem, validateCertificationPath: validateCertificationPath$1 } = common;
...
async function verifyAndroidKeyAttestation$1(ctapCredResp, clientDataJSON) {
	...
	const clientDataHash = hash$3('SHA256', clientDataJSON);
	...
	const ansiKeyPad = Buffer.concat([Buffer([0x00, 0x04]), coseKey.get(COSE_KEYS$1.x), coseKey.get(COSE_KEYS$1.y)]);
...
async function verifyAndroidSafetyNeyAttestation(ctapCredResp, clientDataJSON) {
	...
	const clientDataHash = hash$2('SHA256', base64url$2.toBuffer(clientDataJSON));
	const nonceBase = Buffer.concat([ctapCredResp.authData, clientDataHash]);
	const nonceBuffer = hash$2('SHA256', nonceBase);
...
const base64url$1 = require$$0$1;
const elliptic = require$$1;
const nodeRSA = require$$2$1;
const cbor$1 = require$$3;
const { hash: hash$1, parseAuthData, COSEECDHAtoPKCS } = common;
const { COSE_ALG_HASH, COSE_KEYS, COSE_KTY, COSE_CRV, COSE_RSA_SCHEME } = cose;
...
const base64url = require$$0$1;
const cbor = require$$3;
...
async function verifyAuthenticatorAttestationResponse(webAuthnResponse) {
	...
	if (ctapMakeCredResp.fmt === 'fido-u2f') 
		...
		verification = await noneAttestation(ctapMakeCredResp);
...
const verify = async ({ session, data: webAuthnResp }) => {
	...
	const clientData = JSON.parse(require$$0$1.decode(webAuthnResp.response.clientDataJSON));
	...
	if (webAuthnResp.response.attestationObject !== undefined) {
		...
		result = await helpers.verifyAuthenticatorAssertionResponse(webAuthnResp, authenticators);
		if(result.verified) {
			result.status = 'ok';
			return result;
		}

...
const {
  hash, parseAuthData, verifySignature, COSEECDHAtoPKCS, base64ToPem, getCertificationInfo,
} = require('./common');
const {
  COSE_ALG_HASH, COSE_KEYS, COSE_KTY, COSE_CRV, COSE_RSA_SCHEME,
} = require('./cose');
...
async function verifyPackedAttestation(ctapCredentialResponse, clientDataJSON) {
  ...
  if (ctapCredentialResponse.attStmt.ecdaaKeyId) { throw new Error('ECDAA not implemented yet'); } else {
    ...
    if (publicKeyCose.get(COSE_KEYS.kty) === COSE_KTY.EC2) {
      ...
      const ec = new EC(COSE_CRV[publicKeyCose.get(COSE_KEYS.crv)]);

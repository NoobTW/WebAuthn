...
async function verifyAuthenticatorAttestationResponse(webAuthnResponse) {
  ...
  let verification;
  if (ctapMakeCredResp.fmt === 'fido-u2f') { verification = await verifyU2FAttestation(ctapMakeCredResp, clientDataJSON); } else if (ctapMakeCredResp.fmt === 'packed') { verification = await verifyPackedAttestation(ctapMakeCredResp, clientDataJSON); } else if (ctapMakeCredResp.fmt === 'android-key') { verification = await verifyAndroidKeyAttestation(ctapMakeCredResp, clientDataJSON); } else if (ctapMakeCredResp.fmt === 'android-safetynet') { verification = await verifyAndroidSafetyNetAttestation(ctapMakeCredResp, clientDataJSON); } else if (ctapMakeCredResp.fmt === 'none') { verification = await noneAttestation(ctapMakeCredResp, clientDataJSON); }
